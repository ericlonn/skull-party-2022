...
const SHARD = preload("res://scripts/shards/Shard.tscn")
...
func _ready() -> void:
	if get_parent() is Sprite:
		...
		for t in triangles:
			...
			shard.get_node("Polygon2D").position = -center - Vector2(_rect.size)
			shard.get_node("Polygon2D").uv = points
			...
			shard.get_node("CollisionPolygon2D").position = -center - Vector2(_rect.size)
